...
vector<Interval> IntervalHelper::find_union(vector<Interval> &i1,
    ...
    for(auto ind:i1) v.push_back({ind.bottom,0});
    for(auto ind:i1)v.push_back({ind.top,1});
    ...
    for(auto pos:v)
    {
        if(pos.second == 1) //1 denotes, closing of an interval,so count of the ones inside our open list has to be decreased by 1
        {
            in--;
        }
        else
        ...
        if(in == 0)
        {
            //if no more intervals open or in => its a closed interval, so push in the answer!
